...
import { saveGame, loadGame, gameExists, createUser } from '../db';
import { getChapterByDay, CHAPTERS } from '../engine/storySystem';
import {
  checkDailyEvent,
  applyEventEffects,
  getAvailableChoices,
  getAvailableEventsForState,
} from '../engine/eventSystem';
import {
  dailyFinanceSettlement,
  monthlyFinanceSettlement,
  upgradeOffice,
  getFinanceSummary,
  getUpgradeCost,
  getOfficeName,
...
router.post('/create', (req: Request, res: Response) => {
  try {
    const { userId, characterName, companyName, industry } = req.body as CreateGameRequest;
    ...
    // 创建用户
    createUser(userId, characterName);
...
router.get('/:userId', (req: Request, res: Response) => {
  try {
    ...
    // 附加计算信息（不保存）
    const levelProgress = getLevelProgress(state.character);
    const financeSummary = getFinanceSummary(state);
    const chapter = getChapterByDay(state.currentDay);
    const learnableSkills = getLearnableSkills(state);
...
// ============================================================
router.post('/:userId/action', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const { action, choiceId, params } = req.body as ActionRequest;
    ...
    switch (action) {
      case 'rest':
        newState = restAction(newState);
        break;

      case 'work':
        newState = workAction(newState);
        break;

      case 'network':
        newState = networkAction(newState);
        break;

      case 'upgrade_office':
        newState = upgradeOffice(newState);
        break;

      case 'hire':
        newState = hireAction(newState);
        break;

      case 'marketing_campaign':
        newState = marketingAction(newState);
        break;

      case 'develop_product':
        newState = developProductAction(newState);
        break;
      ...
      case 'take_loan': {
        ...
        newState = takeLoan(newState, amount);
        break;
      }

      case 'research_market':
        newState = researchMarketAction(newState);
        break;

      case 'train_skill':
        newState = trainSkillAction(newState);
        break;
      ...
      case 'handle_event': {
        ...
        // 检查属性要求
        const availableChoices = getAvailableChoices(event, newState.character.stats);
        ...
        // 应用事件效果
        newState = applyEventEffects(newState, event, choice);
...
// ============================================================
router.get('/:userId/events', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const state = loadGame(userId);
    ...
    // 检查今天是否触发新事件
    const event = checkDailyEvent(state);
    ...
    if (event) {
      ...
      const availableChoices = getAvailableChoices(event, state.character.stats);
      res.json({
        success: true,
        data: {
          newEvent: event,
...
// ============================================================
router.post('/:userId/skill', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const { skillId } = req.body as LearnSkillRequest;
    ...
    const newState = learnSkill(state, skillId);
    ...
    saveGame(userId, achievementResult.state);
...
// ============================================================
router.post('/:userId/next-day', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const state = loadGame(userId);
    ...
    // 更新章节
    const newChapter = getChapterByDay(newState.currentDay);
    ...
    // 每日财务结算
    newState = dailyFinanceSettlement(newState);

    // 更新倦怠
    newState = updateBurnout(newState);

    // 每月结算（每30天）
    if (newState.currentDay % 30 === 0) {
      newState = monthlyFinanceSettlement(newState);
    }

    // 检查随机事件
    const event = checkDailyEvent(newState);
    if (event) {
...
// ============================================================
router.get('/:userId/skills', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const state = loadGame(userId);
...
// ============================================================
router.get('/chapters/info', (_req: Request, res: Response) => {
  res.json({
    success: true,
    data: CHAPTERS,
  });
...
/** 获取技能学习失败原因 */
function getSkillFailureReason(state: GameState, skillId: string): string {
  const skill = state.skills.find(s => s.id === skillId);
  if (!skill) return '技能不存在';

  if (skill.currentLevel >= skill.maxLevel) return '该技能已达到最高等级';
...
/** 处理寻求投资行动 */
function handleSeekInvestment(state: GameState): GameState {
  let newState = { ...state };
