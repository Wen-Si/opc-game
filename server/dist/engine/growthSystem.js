...
exports.ACHIEVEMENTS = exports.SKILL_TREE = void 0;
exports.addExperience = addExperience;
exports.getExpForLevel = getExpForLevel;
exports.getLevelProgress = getLevelProgress;
exports.learnSkill = learnSkill;
exports.getLearnableSkills = getLearnableSkills;
exports.getSkillTreeByCategory = getSkillTreeByCategory;
exports.initializeSkillTree = initializeSkillTree;
exports.updateBurnout = updateBurnout;
exports.restAction = restAction;
exports.workAction = workAction;
exports.networkAction = networkAction;
exports.developProductAction = developProductAction;
exports.marketingAction = marketingAction;
exports.hireAction = hireAction;
exports.researchMarketAction = researchMarketAction;
exports.trainSkillAction = trainSkillAction;
...
function learnSkill(state, skillId) {
    ...
    if (skill.effects) {
        ...
        if (skill.effects.leadership)
            newStats.leadership = clamp(newStats.leadership + skill.effects.leadership);
        if (skill.effects.technical)
            newStats.technical = clamp(newStats.technical + skill.effects.technical);
        if (skill.effects.marketing)
            newStats.marketing = clamp(newStats.marketing + skill.effects.marketing);
        if (skill.effects.finance)
            newStats.finance = clamp(newStats.finance + skill.effects.finance);
        if (skill.effects.creativity)
            newStats.creativity = clamp(newStats.creativity + skill.effects.creativity);
        if (skill.effects.resilience)
            newStats.resilience = clamp(newStats.resilience + skill.effects.resilience);
        if (skill.effects.networking)
            newStats.networking = clamp(newStats.networking + skill.effects.networking);
 ...
 * 获取可学习的技能列表
 */
function getLearnableSkills(state) {
 ...
 * 获取技能树按类别分组
 */
function getSkillTreeByCategory() {
    const result = {
        leadership: [],
        technical: [],
        marketing: [],
    ...
    for (const skill of exports.SKILL_TREE) {
...
function initializeSkillTree() {
    return exports.SKILL_TREE.map(skill => ({ ...skill, currentLevel: 0 }));
}
// ============================================================
// 疲劳和倦怠系统
// ============================================================
 ...
 * 研发产品行动
 */
function developProductAction(state) {
    let newState = { ...state };
    newState.character = { ...newState.character };
    newState.company = { ...newState.company };
    // 消耗精力
 ...
 * 营销推广行动
 */
function marketingAction(state) {
    let newState = { ...state };
    newState.character = { ...newState.character };
    newState.company = { ...newState.company };
    // 消耗精力
 ...
 * 市场调研行动
 */
function researchMarketAction(state) {
    let newState = { ...state };
    newState.character = { ...newState.character };
    // 消耗精力
 ...
 * 训练技能行动（消耗精力获取经验）
 */
function trainSkillAction(state) {
    let newState = { ...state };
    newState.character = { ...newState.character };
    // 消耗精力
    newState.character.energy = Math.max(0, newState.character.energy - 20);
...
exports.ACHIEVEMENTS = [
    { id: 'first_client', name: '第一个客户', description: '获得第一个付费客户', condition: (s) => s.company.clients >= 1 },
    { id: 'ten_clients', name: '客户满十', description: '累计拥有10个客户', condition: (s) => s.company.clients >= 10 },
    { id: 'first_profit', name: '首次盈利', description: '公司首次实现月度盈利', condition: (s) => s.company.totalProfit > 0 },
    { id: 'survive_30', name: '满月', description: '存活30天', condition: (s) => s.currentDay >= 30 },
    { id: 'survive_90', name: '季度', description: '存活90天', condition: (s) => s.currentDay >= 90 },
    { id: 'survive_180', name: '半年', description: '存活180天', condition: (s) => s.currentDay >= 180 },
    { id: 'survive_365', name: '一年', description: '存活365天', condition: (s) => s.currentDay >= 365 },
    { id: 'cash_100k', name: '十万富翁', description: '现金储备超过10万', condition: (s) => s.company.cash >= 100000 },
    { id: 'cash_1m', name: '百万富翁', description: '现金储备超过100万', condition: (s) => s.company.cash >= 1000000 },
    { id: 'team_5', name: '小团队', description: '团队规模达到5人', condition: (s) => s.company.teamSize >= 5 },
    { id: 'team_15', name: '大团队', description: '团队规模达到15人', condition: (s) => s.company.teamSize >= 15 },
    { id: 'quality_80', name: '品质保证', description: '产品质量达到80', condition: (s) => s.company.productQuality >= 80 },
    { id: 'market_50', name: '半壁江山', description: '市场占有率达到50%', condition: (s) => s.company.marketShare >= 50 },
    { id: 'level_10', name: '老练创业者', description: '角色等级达到10级', condition: (s) => s.character.level >= 10 },
    { id: 'reputation_80', name: '声名远播', description: '声望达到80', condition: (s) => s.character.reputation >= 80 },
    { id: 'projects_10', name: '项目达人', description: '完成10个项目', condition: (s) => s.company.completedProjects >= 10 },
    { id: 'comeback', name: '浴火重生', description: '在现金低于1万后恢复到10万以上', condition: (s) => s.company.cash >= 100000 && s.gameLog.some(l => l.message.includes('现金储备不足')) },
];
/**
 * 检查并解锁成就
 */
