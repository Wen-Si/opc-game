...
exports.dailyFinanceSettlement = dailyFinanceSettlement;
exports.monthlyFinanceSettlement = monthlyFinanceSettlement;
exports.calculateMonthlyIncome = calculateMonthlyIncome;
...
exports.upgradeOffice = upgradeOffice;
exports.getOfficeName = getOfficeName;
 ...
 * 每日财务结算
 */
function dailyFinanceSettlement(state) {
 ...
 * 每月财务结算
 */
function monthlyFinanceSettlement(state) {
    const newState = { ...state };
 ...
 * 基于客户数、产品质量、市场占有率
 */
function calculateMonthlyIncome(company) {
    // 基础收入 = 客户数 * 基础客单价
    ...
    const income = Math.floor(company.clients * basePricePerClient * qualityMultiplier * marketMultiplier * teamEfficiency);
    return Math.max(0, income);
}
/**
 * 计算月支出
 */
function calculateMonthlyExpense(company) {
    let totalExpense = 0;
    // 办公室租金
 ...
 * 计算每日精力恢复
 */
function calculateDailyEnergyRecovery(state) {
    let recovery = 3; // 基础恢复
    // 心理健康高时恢复更多
    if (state.character.mentalHealth > 70)
        recovery += 2;
    else if (state.character.mentalHealth < 30)
        recovery -= 1;
    // 倦怠风险高时恢复减少
 ...
 * 计算贷款月供
 */
function calculateLoanPayment(state) {
    // 简化处理：从gameLog中查找贷款记录
    const loanLogs = state.gameLog.filter(log => log.message.includes('[贷款]'));
...
//# sourceMappingURL=financeSystem.js.map