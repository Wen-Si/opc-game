...
exports.checkChoiceRequirements = checkChoiceRequirements;
exports.getAvailableChoices = getAvailableChoices;
exports.getAvailableEventsForState = getAvailableEventsForState;
exports.applyEffects = applyEffects;
const storySystem_1 = require("./storySystem");
/** 每日事件触发概率（基础值） */
 ...
 * 每日事件检查 - 判断今天是否触发事件
 */
function checkDailyEvent(state) {
    if (state.isGameOver)
        return null;
    // 如果有待处理事件，不触发新事件
    ...
    // 检查是否应该触发主线事件
    const mainEvents = (0, storySystem_1.getMainEvents)(chapter.number);
    const unplayedMainEvents = mainEvents.filter(e => !state.eventHistory.includes(e.id));
    // 每隔 MAIN_EVENT_INTERVAL 天触发主线事件
    if (unplayedMainEvents.length > 0 &&
    ...
    if (Math.random() < randomChance) {
        const randomEvents = (0, storySystem_1.getRandomEvents)(chapter.number);
        const available = randomEvents.filter(e => !state.eventHistory.includes(e.id));
 ...
 * 基于游戏状态调整概率
 */
function calculateEventChance(state) {
 ...
 * 应用事件选择的效果到游戏状态
 */
function applyEventEffects(state, event, choice) {
    ...
    if (effects.stats) {
        if (effects.stats.leadership)
            newStats.leadership = clamp(newStats.leadership + effects.stats.leadership);
        if (effects.stats.technical)
            newStats.technical = clamp(newStats.technical + effects.stats.technical);
        if (effects.stats.marketing)
            newStats.marketing = clamp(newStats.marketing + effects.stats.marketing);
        if (effects.stats.finance)
            newStats.finance = clamp(newStats.finance + effects.stats.finance);
        if (effects.stats.creativity)
            newStats.creativity = clamp(newStats.creativity + effects.stats.creativity);
        if (effects.stats.resilience)
            newStats.resilience = clamp(newStats.resilience + effects.stats.resilience);
        if (effects.stats.networking)
            newStats.networking = clamp(newStats.networking + effects.stats.networking);
    }
    // 也支持直接在 effects 中指定角色能力属性
    if (effects.leadership)
        newStats.leadership = clamp(newStats.leadership + effects.leadership);
    if (effects.technical)
        newStats.technical = clamp(newStats.technical + effects.technical);
    if (effects.marketing)
        newStats.marketing = clamp(newStats.marketing + effects.marketing);
    if (effects.finance)
        newStats.finance = clamp(newStats.finance + effects.finance);
    if (effects.creativity)
        newStats.creativity = clamp(newStats.creativity + effects.creativity);
    if (effects.resilience)
        newStats.resilience = clamp(newStats.resilience + effects.resilience);
    if (effects.networking)
        newStats.networking = clamp(newStats.networking + effects.networking);
...
function checkChoiceRequirements(choice, stats) {
    ...
    if (reqs.leadership && stats.leadership < reqs.leadership)
        return false;
    if (reqs.technical && stats.technical < reqs.technical)
        return false;
    if (reqs.marketing && stats.marketing < reqs.marketing)
        return false;
    if (reqs.finance && stats.finance < reqs.finance)
        return false;
    if (reqs.creativity && stats.creativity < reqs.creativity)
        return false;
    if (reqs.resilience && stats.resilience < reqs.resilience)
        return false;
    if (reqs.networking && stats.networking < reqs.networking)
        return false;
    return true;
}
/**
 * 获取事件中可用的选择（满足属性要求的）
 */
function getAvailableChoices(event, stats) {
    return event.choices.filter(choice => checkChoiceRequirements(choice, stats));
}
/**
 * 获取当前章节的可用事件列表
 */
function getAvailableEventsForState(state) {
    const chapter = (0, storySystem_1.getChapterByDay)(state.currentDay);
    const allChapterEvents = (0, storySystem_1.getChapterEvents)(chapter.number);
